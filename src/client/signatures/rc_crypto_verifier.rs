...
#[cfg(test)]
mod tests {
    use super::{Collection, RcCryptoVerifier, Verification};
    use env_logger;
    ...
    fn verify_signature(
        ...
        let rc_crypto_verifier = RcCryptoVerifier {};
        ...
        if should_fail {
            assert!(rc_crypto_verifier.verify(&collection).is_err())
        } else {
            match rc_crypto_verifier.verify(&collection) {
                Err(err) => {
                    println!("{:?}", err);
                    assert!(false)
                }
